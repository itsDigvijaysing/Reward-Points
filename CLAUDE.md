...
**Migration guard test**: `app/src/androidTest/.../MigrationTest.kt` (uses `androidx.room:room-testing` + `MigrationTestHelper`) builds a v1 DB and validates migrating up to the current version against the exported `app/schemas/*.json`. It fails if a migration is missing or wrong. Run on a device/emulator (Waydroid): `./gradlew connectedDebugAndroidTest`. **Run it after every `DB_VERSION` bump.**

**Build env requirements**: gradle 8.13 requires **JDK 17** (not 24+). Android SDK needs platforms `android-35`+`android-36` and build-tools `35.0.0`+`36.0.0` installed via `sdkmanager`. Put `sdk.dir=...` in a `local.properties` at the repo root, or export `ANDROID_HOME`. Last verified clean build: 2026-06-10 (`app-debug.apk` ~29 MB; 31/31 JVM unit tests pass via `./gradlew :app:testDebugUnitTest`). JVM tests now include coroutine/ViewModel tests (`kotlinx-coroutines-test`): `RankUpNotifierTest`, `AgentContextBuilderTest`, `HistoryViewModelTest`, `SecretStorageRecoveryTest`, `QuoteRepositoryTest` alongside the `rpg/` logic tests. Instrumented `MigrationTest` runs separately via `connectedDebugAndroidTest`.
